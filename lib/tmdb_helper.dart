...
class TMDBHelper {
  ...
  Future<Map<String, dynamic>> getGenre(int genreId, bool onTVPage) async {
    await dotenv.load(fileName: ".env");
    var url = Uri.https("api.themoviedb.org", "/3/discover/${onTVPage ? 'tv' : 'movie'}", {'with_genres': genreId.toString(), 'language': 'en-US', 'api_key': dotenv.env['TMDB_API_KEY']});
    var response = await http.get(url);
  ...
  Future<Map<String, dynamic>> getNewThisMonth(bool onTVPage) async {
    await dotenv.load(fileName: ".env");
    var url = Uri.https("api.themoviedb.org", "/3/discover/${onTVPage ? 'tv' : 'movie'}", {
      if (onTVPage)
        'first_air_date.gte': DateTime.now().subtract(const Duration(days: 30)).toString().split(' ')[0],
        'first_air_date.lte': DateTime.now().toString().split(' ')[0],
      if (!onTVPage)
        'primary_release_date.gte': DateTime.now().subtract(const Duration(days: 30)).toString().split(' ')[0],
  ...
  Future<Map<String, dynamic>> getReturningThisWeek() async {
    ...
    var url = Uri.https("api.themoviedb.org", "/3/discover/tv", {
      'air_date.gte': DateTime.now().subtract(const Duration(days: 30)).toString().split(' ')[0],
